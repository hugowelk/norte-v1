...
- \`revealed_top_3\`: array of 3 value keys, in rank order.
- \`revealed_full_ranking\`: array of all 8 value keys, ranked highest to lowest.
- \`aspirational_top_3\`: array of 3 value keys, in the order the user picked them (their priority order).
- \`loudest_gap\`: object with \`value\`, \`aspirational_rank\`, \`revealed_rank\`. May be null.
- \`other_gaps\`: array of additional gap objects, each with the same shape as loudest_gap.
- \`name\`, \`current_chapter\`, \`blocker_answer\`, \`blocker_custom_text\`, \`wont_give_up\`: post-paywall fields.
