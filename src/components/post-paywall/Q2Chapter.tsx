...
export function Q2Chapter() {
  ...
  const examples = t('postPaywall.q2.examples', { returnObjects: true }) as string[];
    ...
    <PostPaywallLayout step={1}>
      ...
      <div className="space-y-2">
        <div className="relative">
          <textarea rows={4} maxLength={MAX} value={value} onChange={e => setValue(e.target.value)} className="w-full px-4 py-3 rounded-lg border-2 border-border bg-card focus:border-primary focus:outline-none font-body text-base text-foreground resize-none" />
          {value.length > 0 && (
        ...
        <div>
          <button type="button" onClick={() => setShowExamples(v => !v)} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
            <span>{t('postPaywall.q2.seeExamples')}</span>
